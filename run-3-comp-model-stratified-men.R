...
dt0 <- list(
  ...
  str = as.numeric(sub$totnewy3[sub$agrp == "16-24" & sub$rsex == "Male" & !is.na(sub$tested)]), # stratified by number of new partners
#  str = 1 + 0*(as.numeric(sub$totnewy3[sub$agrp == "16-24" & sub$rsex == "Male" & !is.na(sub$tested)])), # unstratified
...
arrows(0.9,0.6,3.8,0.6,angle=90,code=3,length=0.02, col='darkgreen')
arrows(0.2,0.5,2.5,0.5,angle=90,code=3,length=0.02, col='blue')
arrows(2.9,0.40,8.8,0.40,angle=90,code=3,length=0.02, col='red')
points(1.8,0.6,pch=16, col='darkgreen') # men aged 16-24 reporting 0 new partners
points(0.8, 0.5, pch=16, col='blue') # men aged 16-24 reporting 1 new partner
points(5.1, 0.4, pch=16, col='red') # men aged 16-24 reporting 2+ new partners
# unstratified:
# ph <- hist(100*op0$prev[,1], plot=FALSE)
# lines(rep(ph$breaks, each=2), c(0, rep(ph$density, each=2), 0))
# arrows(1.5,0.55,3.4,0.55,angle=90,code=3,length=0.02)
# points(2.3, 0.55, pch=16) # men aged 16-24 reporting 1 new partner
